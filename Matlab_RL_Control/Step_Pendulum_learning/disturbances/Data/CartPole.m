classdef CartPole < handle
    ...
    methods
        ...
        function reset(obj)
            obj.theta = 0;  %start -3 degrees
            obj.theta_dot = 0; 
            obj.F = 0;
        end
        
        function [reward, next_state,  failed] = take_action(obj, action, U)
            ...
            for step = 1:obj.step_limit
                ...
                obj.F = obj.pid() + U + obj.noise_scalar*normrnd(0,1);
                ...
                if new_sign ~= old_sign && cross_flag == 0 && step > 2
                    rise_time = obj.time(step-1);
                    cross_flag = 1;
                end
            end
            ...
            obj.state(1) = obj.peak;
            obj.state(2) = obj.tr;
            obj.state(3) = obj.norm;
            
            obj.reward = -(max_theta + rise_time + sum(abs(obj.theta_hist)));
            reward = obj.reward; 
...
function [Kp, Ki, Kd] = update_gain(action, in_gains, d)
    ...
    delta = d;
    if action == 1
        Kp1 = in_gains(1) + delta*in_gains(1); 
    elseif action == 2
        Kp1 = in_gains(1) - delta*in_gains(1); 
    elseif action == 3 
        Ki1 = in_gains(2) + delta*in_gains(2); 
    elseif action == 4
        Ki1 = in_gains(2) - delta*in_gains(2); 
    elseif action == 5 
        Kd1 = in_gains(3) + delta*in_gians(3); 
    elseif action == 6
        Kd1 = in_gains(3) - delta*in_gains(3); 
    elseif action == 7
        Kp1 = in_gains(1); 
        Ki1 = in_gains(2); 
        Kd1 = in_gains(3);

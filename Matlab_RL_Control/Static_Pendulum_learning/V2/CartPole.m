classdef CartPole < handle
    ...
    methods
        ...
        function reset(obj)
            obj.theta = ((5*pi/180)*(rand()- 0.5));  %random start max 1deg deviation
            obj.theta_dot = 0; 
            obj.theta_hist = zeros(obj.episode_limit, 1);
            obj.F = 0;
        ...
        function r = get_reward(obj) 
            integral = 1/(sum(abs(obj.theta_hist))*obj.tau);
            variance = 1/((max(obj.theta_hist*10))^2);
            r = obj.reward_weights(1)*integral + obj.reward_weights(2)*variance;
        end

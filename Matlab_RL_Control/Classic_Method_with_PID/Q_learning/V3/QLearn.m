classdef QLearn < handle
    ...
    methods
        function obj = QLearn(num_state_variables, state_dimen ,num_actions, epsilon_decay_start, epsilon_decay_finish)
            ...
            % Create Q-table 
            obj.Q = zeros(obj.state_dimen(1), obj.state_dimen(2), obj.state_dimen(3), obj.state_dimen(4), obj.state_dimen(5), obj.n_a);
        ...
        function a = action(obj,state)
            ...
            if rand() < obj.epsilon
                ...
                for action = 1:obj.n_a
                    ...
                    val = obj.Q(state(1), state(2), state(3), state(4), state(5), action);
                    if val > max_val
                        max_action = action;
                        max_val = val;
                    end
